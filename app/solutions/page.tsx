    ...
    >
      ...
      <article className="section-frame rounded-3xl p-6 md:p-8">
        ...
        <ol className="mt-5 grid gap-3 text-sm md:grid-cols-2">
          <li className="rounded-xl border border-[var(--border)] p-4">1. Load events arrive from EDI, email, and customer portals.</li>
          <li className="rounded-xl border border-[var(--border)] p-4">2. AI flags lane risk and recommends recovery actions.</li>
          <li className="rounded-xl border border-[var(--border)] p-4">3. Managers approve cost-impacting decisions.</li>
          <li className="rounded-xl border border-[var(--border)] p-4">4. Dispatch executes and customer updates are logged.</li>
          <li className="rounded-xl border border-[var(--border)] p-4">5. POD docs are validated for billing readiness.</li>
          <li className="rounded-xl border border-[var(--border)] p-4">6. Finance receives a cleaner packet faster.</li>
        </ol>
        <div className="mt-6">
      ...
      <article className="section-frame rounded-3xl p-6 md:p-8">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          <div className="rounded-xl border border-[var(--border)] p-4 text-sm">TMS and dispatch tools: McLeod, Trimble TMW, Turvo, Rose Rocket, Axon.</div>
          <div className="rounded-xl border border-[var(--border)] p-4 text-sm">ELD and telematics: Samsara, Motive, Geotab, Omnitracs, Garmin.</div>
          <div className="rounded-xl border border-[var(--border)] p-4 text-sm">Load feeds: DAT, Truckstop, EDI 204/214/990, broker APIs.</div>
          <div className="rounded-xl border border-[var(--border)] p-4 text-sm">Accounting/back office: QuickBooks, Sage, PCS, ERP and webhook exports.</div>
        </div>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href="/integrations" className="rounded-full border border-[var(--border)] px-5 py-2.5 text-sm font-semibold">
            See Integrations
          </Link>
          <Link href="/book-demo" className="rounded-full bg-[var(--accent)] px-5 py-2.5 text-sm font-semibold text-[#032d26]">
