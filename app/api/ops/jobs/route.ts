...
import { listDispatchJobs } from "@/lib/platform/job-store";
...
export async function GET(request: Request) {
  ...
  const jobs = await listDispatchJobs(companyId, Number.isFinite(limit) ? Math.max(1, Math.min(limit, 100)) : 25);
  return NextResponse.json({ ok: true, companyId, jobs });
}
